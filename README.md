...
from **Haiku → Sonnet → Opus → Mythos**. Left alone it forages the raspberry bush: it stretches up, plucks a
berry and lobs it over its head into the basket on its back. Levels keep climbing long after the last
evolution, unlocking ten outfits from a hard hat to a crimson cape.
...
| press `1`–`5` (or click the tray) then click | drop a token / coffee / bug / commit / raspberry |
| leave it alone | it forages the bush, wanders, and naps |
| `/` | focus the prompt |

Commands: `/help` `/feed <kind>` `/pet` `/status` `/name <name>` `/wear <outfit>` `/outfits` `/sleep` `/wake` `/stages` `/save` `/reset confirm` `/clear`

Outfits unlock by level and auto-equip: `hardhat` lv 4, `wizard` lv 7, `party` lv 10, `headphones` lv 12,
`beanie` lv 14, `shades` lv 16, `halo` lv 19, `antenna` lv 22, `cape` lv 26. `/outfits` shows the wardrobe
and `/wear none` takes it all off.
